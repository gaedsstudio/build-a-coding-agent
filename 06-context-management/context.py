...
def estimate_tokens(value: Any) -> int:
    ...
    Real tokenizers are provider/model specific. Four characters per token is
    intentionally approximate; the important lesson here is budgeting.
    """
...
class ContextWindow:
    """Keep pinned task context plus the newest complete tool exchanges.

    A turn is an assistant tool-call message together with every tool result
    produced for that message. Turns are kept or dropped as a unit so the chat
    protocol never sees a tool result without its matching tool call.
    """
    ...
    def messages(self) -> list[Message]:
        ...
        for turn in reversed(self._turns):
            ...
            if turn_tokens <= remaining - selected_tokens:
                ...
                continue
            # Do not skip a newer turn to squeeze in an older one.
            break

        selected = list(reversed(selected_reversed))
        flattened = [message for turn in selected for message in turn]
        estimated = base_tokens + selected_tokens
        self._last_report = ContextReport(
            budget_tokens=self.max_tokens,
